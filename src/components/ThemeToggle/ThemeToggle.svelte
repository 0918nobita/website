<script lang="ts">
  ...
  let { currentlyAppliedTheme, themeSetting = $bindable() }: Props = $props();
</script>

<aside class={styles.container}>
  {#if currentlyAppliedTheme === "light"}
    <svg
      class={styles.sunOrMoon}
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 512 512"
      role="img"
    >
      <title>今ライトモードで表示されていることを示すアイコン</title>
  ...
  {:else if currentlyAppliedTheme === "dark"}
    <svg
      class={styles.sunOrMoon}
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 384 512"
      role="img"
    >
      <title>今ダークモードで表示されていることを示すアイコン</title>
      <!--!Font Awesome Free 6.5.1 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.-->
      <path
  ...
  <select id="theme-toggle" class={styles.selector} bind:value={themeSetting}>
